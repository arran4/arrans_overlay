...
SRC_URI="
	amd64? (
		!extended? (
			https://github.com/gohugoio/hugo/releases/download/v${PV}/hugo_${PV}_linux-amd64.tar.gz
			-> ${P}-hugo_${PV}_linux-amd64.tar.gz
		)
		extended? (
			https://github.com/gohugoio/hugo/releases/download/v${PV}/hugo_extended_${PV}_Linux-64bit.tar.gz
			-> ${P}-hugo_extended_${PV}_Linux-64bit.tar.gz
		)
	)
	arm? (
		https://github.com/gohugoio/hugo/releases/download/v${PV}/hugo_${PV}_linux-arm.tar.gz
		-> ${P}-hugo_${PV}_linux-arm.tar.gz
	)
	arm64? (
		!extended? (
			https://github.com/gohugoio/hugo/releases/download/v${PV}/hugo_${PV}_linux-arm64.tar.gz
			-> ${P}-hugo_${PV}_linux-arm64.tar.gz
		)
		extended? (
			https://github.com/gohugoio/hugo/releases/download/v${PV}/hugo_extended_${PV}_linux-arm64.tar.gz
			-> ${P}-hugo_extended_${PV}_linux-arm64.tar.gz
		)
	)
"
S="${WORKDIR}"
