...
src_unpack() {
  if use amd64; then
    unpack "${DISTDIR}/${P}-pagefind-v1.2.0-x86_64-unknown-linux-musl.tar.gz" || die "Can't unpack archive file"
  fi
  if use amd64; then
    unpack "${DISTDIR}/${P}-pagefind_extended-v1.2.0-x86_64-unknown-linux-musl.tar.gz" || die "Can't unpack archive file"
  fi
  if use arm64; then
    unpack "${DISTDIR}/${P}-pagefind-v1.2.0-aarch64-unknown-linux-musl.tar.gz" || die "Can't unpack archive file"
  fi
  if use arm64; then
    unpack "${DISTDIR}/${P}-pagefind_extended-v1.2.0-aarch64-unknown-linux-musl.tar.gz" || die "Can't unpack archive file"
  fi
}
