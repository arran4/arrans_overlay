...
src_unpack() {
    if use full; then
      cp "${DISTDIR}/${P}.amd64" 'whisper.medium.llamafile' || die 'failed to move whisper.medium.llamafile'
      chmod +x 'whisper.medium.llamafile' || die 'failed to chmod whisper.medium.llamafile'
    fi
    if use q5k; then
      cp "${DISTDIR}/${P}.${variant}.amd64" 'whisper.medium.q5k.llamafile' || die 'failed to move whisper.medium.q5k.llamafile'
      chmod +x 'whisper.medium.q5k.llamafile' || die 'failed to chmod whisper.medium.q5k.llamafile'
    fi
    if use q8; then
      cp "${DISTDIR}/${P}.${variant}.amd64" 'whisper.medium.q8.llamafile' || die 'failed to move whisper.medium.q8.llamafile'
...
pkg_postinst() {
    ...
    if use full; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/bin/bash -c '/opt/bin/whisper.medium.llamafile  --host 0.0.0.0 --port 51524 --convert'"
      ...
      } > /usr/lib/systemd/system/whisper.medium.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.medium.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /usr/lib/systemd/system/whisper.medium.llamafile.service /etc/systemd/system/whisper.medium.llamafile.service
      ln -s /usr/lib/systemd/system/whisper.medium.llamafile.service /etc/systemd/user/whisper.medium.llamafile.service
      fi
    fi
    if use q5k; then
      doexe "${WORKDIR}/whisper.medium.q5k.llamafile"
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/bin/bash -c '/opt/bin/whisper.medium.q5k.llamafile  --host 0.0.0.0 --port 51524 --convert'"
        echo "WorkingDirectory=/var/lib/whisperfile"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /usr/lib/systemd/system/whisper.medium.q5k.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.medium.q5k.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /usr/lib/systemd/system/whisper.medium.q5k.llamafile.service /etc/systemd/system/whisper.medium.q5k.llamafile.service
      ln -s /usr/lib/systemd/system/whisper.medium.q5k.llamafile.service /etc/systemd/user/whisper.medium.q5k.llamafile.service
      fi
    fi
    if use q8; then
      doexe "${WORKDIR}/whisper.medium.q8.llamafile"
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/bin/bash -c '/opt/bin/whisper.medium.q8.llamafile  --host 0.0.0.0 --port 51524 --convert'"
        echo "WorkingDirectory=/var/lib/whisperfile"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /usr/lib/systemd/system/whisper.medium.q8.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.medium.q8.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /usr/lib/systemd/system/whisper.medium.q8.llamafile.service /etc/systemd/system/whisper.medium.q8.llamafile.service
      ln -s /usr/lib/systemd/system/whisper.medium.q8.llamafile.service /etc/systemd/user/whisper.medium.q8.llamafile.service
      fi
    fi
}
