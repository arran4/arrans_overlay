...
src_unpack() {
    if use full; then
      cp "${DISTDIR}/${P}.amd64" 'whisper.large-v3.llamafile' || die 'failed to move whisper.large-v3.llamafile'
      chmod +x 'whisper.large-v3.llamafile' || die 'failed to chmod whisper.large-v3.llamafile'
    fi
    if use q5k; then
      cp "${DISTDIR}/${P}.${variant}.amd64" 'whisper.large-v3.q5k.llamafile' || die 'failed to move whisper.large-v3.q5k.llamafile'
      chmod +x 'whisper.large-v3.q5k.llamafile' || die 'failed to chmod whisper.large-v3.q5k.llamafile'
    fi
    if use q8; then
      cp "${DISTDIR}/${P}.${variant}.amd64" 'whisper.large-v3.q8.llamafile' || die 'failed to move whisper.large-v3.q8.llamafile'
...
pkg_postinst() {
    ...
    if use full; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/bin/bash -c '/opt/bin/whisper.large-v3.llamafile  --host 0.0.0.0 --port 51524 --convert'"
      ...
      } > /usr/lib/systemd/system/whisper.large-v3.llamafile.service
      ...
      ln -s /usr/lib/systemd/system/whisper.large-v3.llamafile.service /etc/systemd/system/whisper.large-v3.llamafile.service
      ln -s /usr/lib/systemd/system/whisper.large-v3.llamafile.service /etc/systemd/user/whisper.large-v3.llamafile.service
      fi
    fi
    if use q5k; then
      doexe "${WORKDIR}/whisper.large-v3.q5k.llamafile"
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/bin/bash -c '/opt/bin/whisper.large-v3.q5k.llamafile  --host 0.0.0.0 --port 51524 --convert'"
        echo "WorkingDirectory=/var/lib/whisperfile"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /usr/lib/systemd/system/whisper.large-v3.q5k.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.large-v3.q5k.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /usr/lib/systemd/system/whisper.large-v3.q5k.llamafile.service /etc/systemd/system/whisper.large-v3.q5k.llamafile.service
      ln -s /usr/lib/systemd/system/whisper.large-v3.q5k.llamafile.service /etc/systemd/user/whisper.large-v3.q5k.llamafile.service
      fi
    fi
    if use q8; then
      doexe "${WORKDIR}/whisper.large-v3.q8.llamafile"
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/bin/bash -c '/opt/bin/whisper.large-v3.q8.llamafile  --host 0.0.0.0 --port 51524 --convert'"
        echo "WorkingDirectory=/var/lib/whisperfile"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /usr/lib/systemd/system/whisper.large-v3.q8.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.large-v3.q8.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /usr/lib/systemd/system/whisper.large-v3.q8.llamafile.service /etc/systemd/system/whisper.large-v3.q8.llamafile.service
      ln -s /usr/lib/systemd/system/whisper.large-v3.q8.llamafile.service /etc/systemd/user/whisper.large-v3.q8.llamafile.service
      fi
    fi
}
