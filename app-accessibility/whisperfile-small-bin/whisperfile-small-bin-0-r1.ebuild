...
pkg_postinst() {
    ...
    if use full; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/opt/bin/whisper.small.llamafile  --host 0.0.0.0 --port 51524 --convert"
      ...
      } > /etc/systemd/system/whisper.small.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.small.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /etc/systemd/system/whisper.small.llamafile.service /etc/systemd/user/whisper.small.llamafile.service
    fi
  fi
    einfo "Quick guide:"
    if use q5k; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/opt/bin/whisper.small.q5k.llamafile  --host 0.0.0.0 --port 51524 --convert"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        echo "RestartSec=3"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /etc/systemd/system/whisper.small.q5k.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.small.q5k.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /etc/systemd/system/whisper.small.q5k.llamafile.service /etc/systemd/user/whisper.small.q5k.llamafile.service
    fi
  fi
    einfo "Quick guide:"
    if use q8; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/opt/bin/whisper.small.q8.llamafile  --host 0.0.0.0 --port 51524 --convert"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        echo "RestartSec=3"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /etc/systemd/system/whisper.small.q8.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.small.q8.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /etc/systemd/system/whisper.small.q8.llamafile.service /etc/systemd/user/whisper.small.q8.llamafile.service
    fi
  fi
}
