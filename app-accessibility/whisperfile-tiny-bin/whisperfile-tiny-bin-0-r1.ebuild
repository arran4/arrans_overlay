...
src_unpack() {
    if use full; then
      mv "${P}.amd64" 'whisper.tiny.llamafile' or die 'failed to move whisper.tiny.llamafile'
      chmod +x 'whisper.tiny.llamafile' or die 'failed to chmod whisper.tiny.llamafile'
    fi
    if use q5k; then
      mv "${P}.${variant}.amd64" 'whisper.tiny.q5k.llamafile' or die 'failed to move whisper.tiny.q5k.llamafile'
      chmod +x 'whisper.tiny.q5k.llamafile' or die 'failed to chmod whisper.tiny.q5k.llamafile'
    fi
    if use q8; then
      mv "${P}.${variant}.amd64" 'whisper.tiny.q8.llamafile' or die 'failed to move whisper.tiny.q8.llamafile'
      chmod +x 'whisper.tiny.q8.llamafile' or die 'failed to chmod whisper.tiny.q8.llamafile'
    fi
}
...
pkg_postinst() {
    ...
    if use full; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/opt/bin/whisper.tiny.llamafile  --host 0.0.0.0 --port 51524 --convert"
      ...
      } > /etc/systemd/system/whisper.tiny.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.tiny.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /etc/systemd/system/whisper.tiny.llamafile.service /etc/systemd/user/whisper.tiny.llamafile.service
    fi
  fi
    einfo "Quick guide:"
    if use q5k; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/opt/bin/whisper.tiny.q5k.llamafile  --host 0.0.0.0 --port 51524 --convert"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        echo "RestartSec=3"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /etc/systemd/system/whisper.tiny.q5k.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.tiny.q5k.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /etc/systemd/system/whisper.tiny.q5k.llamafile.service /etc/systemd/user/whisper.tiny.q5k.llamafile.service
    fi
  fi
    einfo "Quick guide:"
    if use q8; then
      ...
      einfo "Creating systemd service file..."
      {
        ...
        echo ""
        echo "[Service]"
        echo "ExecStart=/opt/bin/whisper.tiny.q8.llamafile  --host 0.0.0.0 --port 51524 --convert"
        echo "User=whisperfile"
        echo "Group=whisperfile"
        echo "Restart=always"
        echo "RestartSec=3"
        ...
        echo "[Install]"
        echo "WantedBy=default.target"
      } > /etc/systemd/system/whisper.tiny.q8.llamafile.service
      einfo "Service file created at /etc/systemd/system/whisper.tiny.q8.llamafile.service"
      einfo "Making service user-startable..."
      mkdir -p /etc/systemd/user
      ln -s /etc/systemd/system/whisper.tiny.q8.llamafile.service /etc/systemd/user/whisper.tiny.q8.llamafile.service
    fi
  fi
}
