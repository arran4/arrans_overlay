...
SRC_URI="
  amd64? ( https://github.com/arran4/flutter_google_datastore/releases/download/v0.1.28/flutter_google_datastore-0.1.28-linux.AppImage -> ${P}-flutter_google_datastore-${PV}-linux.AppImage )
"

src_unpack() {
  if use amd64; then
    cp "${DISTDIR}/${P}-flutter_google_datastore-${PV}-linux.AppImage" "flutter_google_datastore.AppImage"  || die "Can't copy downloaded file"
  fi
