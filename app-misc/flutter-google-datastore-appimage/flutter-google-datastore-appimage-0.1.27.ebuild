...
SRC_URI="
  amd64? ( https://github.com/arran4/flutter_google_datastore/releases/download/v0.1.27/flutter_google_datastore-0.1.27-linux.AppImage -> ${P}-flutter_google_datastore-0.1.27-linux.AppImage )
"

src_unpack() {
  if use amd64; then
    cp "${DISTDIR}/${P}-flutter_google_datastore-0.1.27-linux.AppImage" "flutter_google_datastore.AppImage"  || die "Can't copy downloaded file"
  fi
  ...
  "./flutter_google_datastore.AppImage" --appimage-extract "usr/share/icons" || die "Failed to extract hicolor icons from app image"
  "./flutter_google_datastore.AppImage" --appimage-extract "*.png" || die "Failed to extract root icons from app image"
}
