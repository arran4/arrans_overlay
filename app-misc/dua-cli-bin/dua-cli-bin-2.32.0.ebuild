...
SRC_URI="
  amd64? (  https://github.com/Byron/dua-cli/releases/download/v2.32.0/dua-v2.32.0-x86_64-unknown-linux-musl.tar.gz -> ${P}-dua-v2.32.0-x86_64-unknown-linux-musl.tar.gz  )  
  arm? (  https://github.com/Byron/dua-cli/releases/download/v2.32.0/dua-v2.32.0-arm-unknown-linux-gnueabihf.tar.gz -> ${P}-dua-v2.32.0-arm-unknown-linux-gnueabihf.tar.gz  )  
  arm64? (  https://github.com/Byron/dua-cli/releases/download/v2.32.0/dua-v2.32.0-aarch64-unknown-linux-musl.tar.gz -> ${P}-dua-v2.32.0-aarch64-unknown-linux-musl.tar.gz  )  
"

src_unpack() {
  if use amd64; then
    unpack "${DISTDIR}/${P}-dua-v2.32.0-x86_64-unknown-linux-musl.tar.gz" || die "Can't unpack archive file"
  fi
  if use arm; then
    unpack "${DISTDIR}/${P}-dua-v2.32.0-arm-unknown-linux-gnueabihf.tar.gz" || die "Can't unpack archive file"
  fi
  if use arm64; then
    unpack "${DISTDIR}/${P}-dua-v2.32.0-aarch64-unknown-linux-musl.tar.gz" || die "Can't unpack archive file"
  fi
}
