...
SRC_URI="
  amd64? (  https://github.com/arran4/rntocase/releases/download/v0.1.1/rntocase_${PV}_linux_amd64.tar.gz -> ${P}-rntocase_${PV}_linux_amd64.tar.gz  )  
  arm64? (  https://github.com/arran4/rntocase/releases/download/v0.1.1/rntocase_${PV}_linux_arm64.tar.gz -> ${P}-rntocase_${PV}_linux_arm64.tar.gz  )  
"
...
src_install() {
  exeinto /opt/bin
  if use amd64; then
    newexe "rnacronym" "rnacronym" || die "Failed to install Binary"
  fi
  if use arm64; then
    newexe "rnacronym" "rnacronym" || die "Failed to install Binary"
  fi
  ...
  if use amd64; then
    newexe "rntolower" "rntolower" || die "Failed to install Binary"
  fi
  if use arm64; then
    newexe "rntolower" "rntolower" || die "Failed to install Binary"
  fi
  ...
  if use amd64; then
    newexe "rntopascal" "rntopascal" || die "Failed to install Binary"
  fi
  if use arm64; then
    newexe "rntopascal" "rntopascal" || die "Failed to install Binary"
  fi
  ...
  if use amd64; then
    newexe "rntoupper" "rntoupper" || die "Failed to install Binary"
  fi
  if use arm64; then
    newexe "rntoupper" "rntoupper" || die "Failed to install Binary"
  fi
