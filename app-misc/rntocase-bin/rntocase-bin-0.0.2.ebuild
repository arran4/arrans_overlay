...
SRC_URI="
  amd64? (  https://github.com/arran4/rntocase/releases/download/v0.0.2/rntocase_${PV}_linux_amd64.tar.gz -> ${P}-rntocase_${PV}_linux_amd64.tar.gz  )  
  arm64? (  https://github.com/arran4/rntocase/releases/download/v0.0.2/rntocase_${PV}_linux_arm64.tar.gz -> ${P}-rntocase_${PV}_linux_arm64.tar.gz  )  
"
...
src_install() {
  exeinto /opt/bin
  if use amd64; then
    newexe "rnreverse" "rntrim" || die "Failed to install Binary"
  fi
  if use arm64; then
    newexe "rnreverse" "rntrim" || die "Failed to install Binary"
  fi
