...
src_unpack() {
  if use amd64; then
    unpack "${DISTDIR}/${P}-gocdm_${PV}_linux_amd64.tar.gz" || die "Can't unpack archive file"
  fi
  if use arm64; then
    unpack "${DISTDIR}/${P}-gocdm_${PV}_linux_arm64.tar.gz" || die "Can't unpack archive file"
  fi
}
...
pkg_postinst() {
  ...
  einfo "[Service]"
  einfo "ExecStart="
  einfo "ExecStart=-/opt/bin/gocdm -login"
  einfo "StandardInput=tty"
  einfo "StandardOutput=tty"
  einfo "StandardError=tty"
  einfo "TTYPath=/dev/%I"
  einfo "TTYReset=yes"
