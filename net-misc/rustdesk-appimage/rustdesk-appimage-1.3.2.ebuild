...
SRC_URI="
  arm64? ( https://github.com/rustdesk/rustdesk/releases/download/1.3.2/rustdesk-1.3.2-aarch64.AppImage -> ${P}-rustdesk-1.3.2-aarch64.AppImage )
  amd64? ( https://github.com/rustdesk/rustdesk/releases/download/1.3.2/rustdesk-1.3.2-x86_64.AppImage -> ${P}-rustdesk-1.3.2-x86_64.AppImage )
"

src_unpack() {
  if use amd64; then
    cp "${DISTDIR}/${P}-rustdesk-1.3.2-x86_64.AppImage" "rustdesk.AppImage"  || die "Can't copy downloaded file"
  fi
  if use arm64; then
    cp "${DISTDIR}/${P}-rustdesk-1.3.2-aarch64.AppImage" "rustdesk.AppImage"  || die "Can't copy downloaded file"
  fi
